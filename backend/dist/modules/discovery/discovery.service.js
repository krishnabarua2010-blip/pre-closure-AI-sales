...
const scraper_service_1 = require("../external/scraper.service");
...
class DiscoveryService {
     ...
     */
    static async discoverAndScorePipeline(businessProfileId, niche, location, count = 10) {
        const bp = await prisma_1.prisma.businessProfile.findUnique({ where: { id: businessProfileId } });
        if (!bp)
            throw new Error("Business profile not found");
        // 1. LEAD DISCOVERY: Google Maps + SERP scraping (replaces Apollo)
        const rawLeads = await scraper_service_1.ScraperService.discoverLeads(niche, location, count);
        const bpContext = {
        ...
        const scoredLeadsPromises = rawLeads.map(async (rawLead) => {
            ...
            let existingLead = await prisma_1.prisma.lead.findFirst({
                where: {
                    name: rawLead.name,
            ...
            if (!existingLead) {
                existingLead = await prisma_1.prisma.lead.create({
                    data: {
                        business_profile_id: bp.id,
                        name: rawLead.name,
            ...
            try {
                // 2a. AI Enrichment
                const enriched = await scraper_service_1.ScraperService.enrichLead(rawLead);
                // 2b. 5-Dimension Scoring
                const { score, tag, breakdown } = scraper_service_1.ScraperService.scoreLead(enriched, niche);
                ...
                // 2e. Create Outreach
                const existingOutreach = await prisma_1.prisma.outreach.findFirst({
                    where: { lead_id: existingLead.id }
                });
                if (!existingOutreach) {
                    await prisma_1.prisma.outreach.create({
                        data: {
     ...
     */
    static async generateOutreachMessage(lead, bp) {
        try {
...
- Make them feel understood, not sold to`
     ...
     */
    static async generateFollowUpSequence(leadId, lead, bp) {
        // Check if sequence already exists
        const existingFollowUps = await prisma_1.prisma.followUp.count({ where: { lead_id: leadId } });
        if (existingFollowUps >= 4)
            return;
        try {
            const response = await openai.chat.completions.create({
                model: 'gpt-4o-mini',
