...
import { CalendarSyncService, WebhookPayload } from './calendar-sync.service';
...
export default async function externalRoutes(fastify: FastifyInstance) {
   ...
   */
  fastify.post('/webhooks/calendar', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      ...
      // 3. Detect Google Calendar
        ...
        provider = 'google';
        event = 'google.sync';
        eventId = (request.headers['x-goog-resource-id'] as string) || '';
      ...
      const syncPayload: WebhookPayload = {
        event,
        provider,
        eventId,
        email,
      ...
      const result = await CalendarSyncService.processWebhookEvent(syncPayload);
