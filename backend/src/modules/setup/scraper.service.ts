...
export class ScraperService {
  ...
  private static async fetchPageText(url: string): Promise<string> {
    try {
  ...
  static async crawlAndExtract(businessProfileId: number, url: string): Promise<any> {
    ...
    try {
      ...
      if (!baseResponse.ok) throw new Error(`Homepage returned status ${baseResponse.status}`);
      ...
      $('a[href]').each((_: any, elem: any) => {
        ...
        if (!absoluteUrl.startsWith(homeOrigin)) return;
...
${cleanedInput}`;
      ...
      const extractedProfile = JSON.parse(rawContent) as ExtractedProfile;

      // 4. Calculate Confidence Score
      let score = 0;
      if (extractedProfile.business_name && extractedProfile.business_name !== 'Pre-Closure AI Client') score += 15;
      if (extractedProfile.industry) score += 10;
      if (extractedProfile.services && extractedProfile.services.length > 0) score += 15;
      if (extractedProfile.pricing && extractedProfile.pricing.length > 0) score += 15;
      if (extractedProfile.target_audience && extractedProfile.target_audience.length > 0) score += 10;
      if (extractedProfile.faqs && extractedProfile.faqs.length > 0) score += 15;
      if (extractedProfile.common_objections && extractedProfile.common_objections.length > 0) score += 10;
      if (extractedProfile.booking_links && extractedProfile.booking_links.length > 0) score += 10;
        ...
        try {
          await prisma.scrapeJob.update({
            ...
            data: {
              ...
              extracted_data: extractedProfile as any
            }
          });
        } catch {}
      }
        ...
        try {
          await prisma.scrapeJob.update({
            ...
            data: {
              status: 'FAILED',
              confidence_score: 0.0,
              extracted_data: fallbackProfile as any
            }
          });
        } catch {}
      }
